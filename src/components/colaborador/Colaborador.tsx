...
export default function ColaboradorCard({ nome, cargo, linkedin, imagem }: ColaboradorProps) {
  return (
    <div className="bg-[#F1EDD2] rounded-2xl shadow-md p-4 w-full max-w-xs hover:shadow-lg transition hover:cursor-pointer hover:scale-105">
      {imagem ? (
        <img
          src={imagem}
          alt={`Foto de ${nome}`}
          className="w-24 h-24 rounded-full mx-auto mb-4 object-cover border-2 border-[#F4BF4F]"
        />
      ) : (
      ...
      <a
        ...
        className="block text-center text-yellow-800 hover:underline text-sm"
      >
        Ver no LinkedIn
      </a>
    </div>
